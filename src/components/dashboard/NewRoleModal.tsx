// src/components/dashboard/NewRoleModal.tsx
'use client';
...
export default function NewRoleModal({ isOpen, onClose, onRoleCreated }: NewRoleModalProps) {
  const [name, setName] = useState('');
  const [primaryDesk, setPrimaryDesk] = useState('helpdesk');
  const [description, setDescription] = useState('');
  const [allowedDesks, setAllowedDesks] = useState<string[]>(['helpdesk']);
  const [assignedModules, setAssignedModules] = useState<string[]>(['clinical-triage', 'clinic-registry']);
  ...
  const handleDeskChange = (desk: string) => {
    ...
    if (!allowedDesks.includes(desk)) {
      setAllowedDesks((prev) => [...prev, desk]);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const created = await createRole({
        ...
        description: description || `Operational desk role for ${name}`,
      ...
      <div className="bg-white rounded-3xl max-w-3xl w-full shadow-2xl overflow-hidden border border-slate-200/80 animate-in fade-in zoom-in-95 duration-200 my-auto flex flex-col max-h-[92vh]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-black text-slate-900 tracking-tight">
                Create Hospital Desk Role
              </h2>
              <p className="text-xs text-slate-500 mt-0.5">
                Define an operational role title, assigned navigation modules, and allowed workstations.
              </p>
            </div>
          </div>
          <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Senior Cashdesk Teller / Billing Supervisor"
        ...
        <div className="px-6 py-4 border-t border-slate-200/70 bg-slate-50 flex items-center justify-between shrink-0">
          <div className="text-[11px] text-slate-500 font-medium hidden sm:block">
            ...
            <span className="font-bold text-slate-800">{name || 'new role'}</span>
          </div>
          ...
          <div className="flex items-center gap-2.5 ml-auto">
            ...
            >
              ...
              <span>{loading ? 'Creating Role...' : 'Create & Register Role'}</span>
            </button>
          </div>
        </div>
      </div>
