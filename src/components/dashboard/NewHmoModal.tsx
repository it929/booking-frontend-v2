// src/components/dashboard/NewHmoModal.tsx
'use client';

import React, { useState } from 'react';
import { createHmoCompany } from '@/lib/api';
import { HmoCompany } from '@/lib/types';
import { X, ShieldCheck, Plus, AlertCircle, RefreshCw } from 'lucide-react';

interface NewHmoModalProps {
  isOpen: boolean;
  onClose: () => void;
  onHmoCreated: (newHmo: HmoCompany) => void;
}

export default function NewHmoModal({
  isOpen,
  onClose,
  onHmoCreated,
}: NewHmoModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newHmo = await createHmoCompany(payload);
      onHmoCreated(newHmo);
      onClose();
      // Reset
      setName('');
      ...
      setError(err instanceof Error ? err.message : 'Failed to register HMO provider');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-black text-slate-900 tracking-tight">
                Accredit New HMO Provider
              </h2>
              <p className="text-xs text-slate-500">
                Register authorized health insurer, policy prefix, and desk contact details
              </p>
            </div>
          </div>
          <button
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  <span>Accredit HMO Provider</span>
                </>
              )}
            </button>
          </div>
